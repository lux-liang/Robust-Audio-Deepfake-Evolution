...
def set_seed(seed, config = None):
    ...
    This function sets seeds for all random number generators used in the codebase:
    - Python's random module (used by RawBoost and other utilities)
    - NumPy's random module (used by RawBoost and data processing)
    - PyTorch's random module (used by model initialization and training)
    - CUDA random module (used by GPU operations)
    
    Args:
        seed: Integer seed value
        config: Configuration dictionary containing cudnn settings
    
    Note:
        - RawBoost uses np.random, so np.random.seed() is critical
        - DataLoader uses torch.Generator, which is controlled by torch.manual_seed()
        - Setting cudnn.deterministic=True sacrifices speed for reproducibility
    """
    if config is None:
    ...
    # NumPy's random module (used by RawBoost and data processing)
    # This is critical for RawBoost reproducibility
    np.random.seed(seed)
    ...
    if torch.cuda.is_available():
        ...
        # Deterministic mode: sacrifices speed for reproducibility
        ...
        # Benchmark mode: should be False when deterministic=True
        torch.backends.cudnn.benchmark = str_to_bool(config["cudnn_benchmark_toggle"])
    
    print(f"[set_seed] Random seed set to {seed}")
    print(f"[set_seed] CUDNN deterministic: {torch.backends.cudnn.deterministic}")
    print(f"[set_seed] CUDNN benchmark: {torch.backends.cudnn.benchmark}")
